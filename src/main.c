...
void main(void)
{
    ...
    // Cargar paletas: pal_bg recibe los primeros 16 bytes, pal_spr los siguientes 16
    pal_bg(palette);
    pal_spr(palette + 16); // Apuntar a la segunda mitad del array (paletas de sprites)
    ...
    while (1)
    {
        ...
        for (y = 0; y < 4; y++)
        {
            for (x = 0; x < 4; x++)
            {
                if (grid[x][y] > 0)
                {
                    ...
                    unsigned char py = 47 + y * 40; // -1 para alinear correctamente
        ...
        // Dibujar cursor (4 sprites en las esquinas)
        {
            ...
            oam_off = oam_spr(px + 32, py, 1, 0, oam_off);
            oam_off = oam_spr(px, py + 32, 1, 0, oam_off);
            oam_off = oam_spr(px + 32, py + 32, 1, 0, oam_off);
